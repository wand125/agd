...
```bash
bun install          # 開発用の依存のみ
bun run check        # 型チェック + テスト
```

`bun test` が見るのは純粋なロジック(トランスクリプト解析・プロンプト検出・tmux 出力の解析)です。
ターミナルに触る部分は手動確認です。詳しくは [CONTRIBUTING.ja.md](CONTRIBUTING.ja.md) を参照してください。

## セキュリティ

agd はターミナルセッションに文字を打ち込めるため、このポートに届くことはシェルに入れることと同義です。
既定では `127.0.0.1` にのみバインドし、トークン無しで外部に出す指定は起動を拒否します。
ネットワークに公開する前と、脆弱性を報告するときは [SECURITY.ja.md](SECURITY.ja.md) を読んでください。
